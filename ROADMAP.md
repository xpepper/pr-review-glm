...
| L2 | ✅ Done (PR #9) | **autopilot merge mode** (non-plugin increment): `--merge human\|auto` on the dev-loop — the loop itself squash-merges the increment PR when gates are green, all active reviews are clean, and the reviewed head is unchanged (headRefOid recorded at assessment, re-fetched and pinned immediately before `gh pr merge`; a moved head re-enters assessment once, a second move stops); merging stays loop-owned and code-governed, never agent-discretion. Default human, auto always an explicit flag; pre-I3 auto = explicit opt-in on the independent review alone; from I3, auto additionally requires the dogfood review (enforced by I3's wiring). The assessment path also ff-only-syncs the PR branch to origin so gates/reviews test the exact head the pin records. Evidence: 131 unit tests (`node --test tests/*.test.mjs`, incl. the merge-mode matrix: auto+dogfood-off merges with post-merge gates; human stops at awaiting-human-merge (default and with dogfood on); auto never merges on blocking findings, fatal review stops, exhausted fixer budget, unknown/unpinnable head, or a twice-moved head) + `node tests/smoke-i1.mjs` + `node tests/smoke-i2.mjs` + `node tests/smoke-l1.mjs` (dry-run green, now surfacing the merge mode). Merging of L2's own PR stayed human (the flag didn't exist until it landed). | L1 |
...
- **2026-09-10 (L2)** — autopilot merge mode landed (PR #9): `--merge human|auto`
  ...
  `tests/dev-loop-cli.test.mjs` (122 → 131). No new runtime facts; no spec changes
  (implemented the already-amended design as-is).
