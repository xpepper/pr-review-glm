...
**Where we are:** I8 + V2 + R45 + S46 complete (0.2.8 released 2026-09-15 under the R45 post-merge flow). **Next:** V3 — the 1.0.0 release decision round (the user's call; V2 ground testing landed as 0.2.7 on 2026-09-14).
...
| V1 | ✅ Done (PR #23) | **Plugin release versioning (semantic):** `plugin.json` `version` follows semver so any user can tell which release they are running — bumped per merged release (pre-1.0: additive → patch, breaking → minor; `1.0.0` when I8 completes), the squash merge tagged `vX.Y.Z` on `main`, and `/z-pr-review status` reports the running version in-chat (read from `plugin.json` at status time, informational only, degrades to "(unknown)" — never a gate input). Design settled 2026-09-11 (journey entries): **every merged increment bumps**, enforced by a new dev-loop `version-bump` gate (`scripts/dev-loop/version.mjs`: fails an increment PR whose version is unchanged vs `origin/main`, fail-closed on git/parse errors, strict `X.Y.Z` shape on both sides); the **loop tags `vX.Y.Z`** in its merge path (`tagMergedRelease`: after squash-merge + checkout + ff-only pull, tag from `plugin.json` on main and push, fail-closed on read/parse/tag/push errors — a tag failure surfaces as a merge-path failure with a precise reason since the merge itself is already done). V1's own merge is tagged `v0.2.0` once by the supervisor as a bootstrap (the running loop predates the code); auto-tagging starts with the increment after V1. V1 itself lands as **0.2.0** — one minor leap acknowledging R1's breaking rename plus I2–I4 landing while the version sat stale at 0.1.0; additive increments after V1 → 0.2.1…. Protocol surfaces untouched (STATUS grammar, command descriptions, machine-summary shape all exactly as main). Review hardening (PR #23 findings, same PR): the merge tail polls GitHub for MERGED (30 × 10s — a queued or briefly-delayed merge still gets tagged instead of exiting unretried; on exhaustion the loop stops loudly, disclosing that the merge may have completed, the release tag was NOT created, and post-merge checks did NOT run), pins `gh pr view`'s `mergeCommit.oid` and refuses to tag when main's HEAD is not this PR's merge commit (another merge landing first fails closed, never tags the wrong commit); `compareVersions` compares digit strings (length-then-lexical) so numeric identifiers beyond 2^53 keep ordering; manifest reads in the gate and tagging tail are fail-closed details, not crashes (unreadable PR manifest → failed version-bump gate; unreadable main manifest → the documented release-tag failure); a failed local-tag delete after a failed push is disclosed for the retry; `readPluginVersion` extracted to `extensions/z-pr-review/version.mjs` with unit coverage (the status version source was the only untested deliverable). Supervisor round (after the fixer budget exhausted on two dogfood P1s, findings validated against the code first): the valid one — the bump gate's `origin/main` baseline can go stale between assessment and merge — fixed with `verifyBumpAtMerge` in the merge path (fresh `git fetch origin main`, re-compare `HEAD:plugin.json` vs `origin/main:plugin.json` immediately before `gh pr merge`, abort fail-closed on staleness: the version-side twin of the headRefOid pin); the other, "tags are skipped when a merge queue takes >2m", was inaccurate as stated (the tail already failed closed with a loud stop, nothing skipped) and was answered with the widened 30-poll window + the honest exhaustion message. Supervisor round 2 (re-run's round-2 dogfood, findings validated against the code first): the re-check now reads the PR head's manifest BY ITS PINNED REMOTE OID (`gh pr view headRefOid` → `git show <oid>:plugin.json`), never the local checkout — the residual fetch→merge window is the same one the head pin accepts by design and is backstopped post-hoc by the tail's merge-commit pin; the version-bump gate moved before the inference-consuming smokes so a bump-less PR fails in seconds; `laneExcerpt` strips terminal control characters from contract-violation reasons; the loop fixer's disclosed scope addition (same-model lane retry on default configs without a fallback model, prompt example fix — the PR #23 flaky-lane blocker) stays with its disclosed-status P2 recorded here. Supervisor round 3 (run-3 findings): the dogfood's two P1s were both real — the fixer's GraphQL `mergePullRequest` mutation used a `headRefOid` input field that does not exist in GitHub's schema (`MergePullRequestInput` takes `expectedHeadOid` — every real merge attempt would have been schema-rejected; the unit tests string-matched the same hallucinated name), and the branch was deleted on `data.mergePullRequest` truthiness before any merge confirmation; fixed to the schema-valid field, `mergeCommit.oid` required as the merge evidence, and branch deletion isolated into `deleteMergedBranch` run by the merge dep ONLY after the tail confirmed MERGED and the tag landed (failure = disclosed warning). Independent P2s folded: terminal CLOSED fails the tail fast instead of polling the full window, the loop stop line says `merge path failed` (the tail can fail after a completed merge), and `verifyBumpAtMerge` shares `gates.mjs`'s `isFullOid`. Supervisor round 3 (three P1s, all one root cause — the reviewed head was pinned only locally): the squash-merge is now GitHub's GraphQL `mergePullRequest` mutation with `expectedHeadOid` set (`squashMergeAtHead`, replacing `gh pr merge --squash --delete-branch`, which merges whatever head GitHub holds — the local pin could lose a seconds-wide race); GitHub rejects the mutation server-side if the head moved, so enforcement is atomic, and the branch is deleted after a successful merge with the failure disclosed (the merge cannot un-do itself); `verifyBumpAtMerge` now fetches `+refs/heads/main:refs/remotes/origin/main` — a bare `git fetch origin main` updates only FETCH_HEAD, so the re-check re-read the stale `origin/main` baseline it existed to re-check. Evidence: 272 unit tests (`node --test tests/*.test.mjs`, incl. the squashMergeAtHead matrix — pinned-mutation call shape, head-moved refusal, id-resolution/unparseable-output fail-closed, branch-delete disclosure — and a refspec regression test) + smoke-i1/i2 (SDK dispatch; smoke-i1 asserts the status `Version:` line against plugin.json) + smoke-l1 (dry-run) green. | — |
...
| I7 | ✅ Done (PR #37) | Gated COMMENT publication: `publish.mjs` posts ONE COMMENT review (`gh api -X POST …/reviews --input -`) of the retained review's settled selection. Authority is code-only and captured before lanes start — `--comment`, or config `autoPostReviews` unless `--no-comment` (extension wiring); model text never selects event, commit, repo, or anchors. Gates before the POST, all fail-closed: draft/closed refusal, self-author refusal (vs `gh api user`), empty-selection skip (no POST), stale head → body-only comment naming both commits (no inline anchors), idempotency marker (`<!-- z-pr-review repo#N@head -->`) scanned against existing reviews (a carrier skips the POST; pagination-capped scans), inline anchors = first ≤50 selected findings re-validated against `pulls/N/files` new-side hunks with the remainder to body "Other notes", a body-size cap under GitHub's limit, a final head re-check immediately before the POST, per-target in-process write serialization, and uncertain-write (5xx/timeout/unparseable response) reconciliation by re-scanning for the marker (found → published-reconciled; absent → fail-closed). Model text is sanitized (flattened, control chars stripped, `<!--` defused) so findings can't forge the marker or chat lines; non-complete reviews publish with a coverage disclosure. The I6 review P2s folded in: `parseSelectionSpec` refuses leading zeros and range bounds before expansion, `inspect` strips terminal control sequences and discloses when the retained review predates the session's last capture. Supervisor fold (dogfood round 2 + independent reviewer, same day): REST lifecycle state normalized before the gate comparisons (the round-2 P1 — `gh api` returns lowercase `open` while the gates compared `OPEN`, so every open-PR publication was refused and uppercase test fixtures masked it; fixtures are now REST-shaped so the happy path is itself the regression test), the base sha pinned alongside the head in the pre-POST re-check (a base advancing without moving the head re-diffs the PR under the validated anchors — an explicit fail-closed refusal instead of a late 422), the review's AbortController signal threaded into publication (cancelled reviews refuse before any gh call and fail closed between the re-check and the POST), stderr bounded by the same byte budget as stdout with byte-true counters on both (string `length` counts UTF-16 units and under-reported multibyte output), a per-inline-comment body cap mirroring the body cap, body-note filenames sanitized like every other model-influenced field (a marker-shaped path could forge the idempotency marker in our own posted body), the dead `authority` parameter removed from `publishReview` (authority is the invocation decision in extension.mjs, not a write-path input), and the stale-head note reworded to name the commits where it names them. Supervisor fold 2 (dogfood round 3 on the folded head): staleness made symmetric in head AND base against the capture (a base advancing under an unchanged head re-diffs the PR the findings were validated against — body-only, naming both heads and both bases), stale publications skip the changed-file fetch entirely (a >500-file PR must not fail a body-only comment), the idempotency scans accept only the authenticated viewer's reviews (deterministic marker text is otherwise forgeable by any PR participant), pagination accepts lists of exactly the entry cap (one beyond-cap probe entry distinguishes at-cap from over-cap), C1 controls (U+0080–U+009F, 8-bit CSI) stripped in both publish sanitization and inspect rendering, capture buffers stop retaining data the moment the byte budget trips, and replacing a select-settled retained review now discloses it (one review is one selection surface by design — the spec's autoPostReviews publishes without an interactive selection; carrying selections across reviews is post-v1). Supervisor fold 3 (dogfood round 5, supervised completion): the idempotency marker now names the CAPTURE binding (head+base) instead of the live head — a stale publication (captured H1, posted after the PR moved) can no longer occupy the new head's slot and suppress a distinct fresh review (round-5 P1, regression-tested with a stateful two-publication fake), and the loop binds the selected increment PR to the authenticated viewer before it feeds the head pin or the auto-merge (`verifyIncrementPrOwnedByViewer` in gateIncrementPr, the assessment inline gate, and findResumablePr — a matching branch name is not provenance; same viewer-ownership posture as the marker scan; round-5 P1). Round-5 P2s recorded riding to I8: uncertain-write reconciliation is a single scan (a stale review listing can still duplicate on rerun), smoke-i3's default target selection can diverge from the gates' increment-scoped selection, and the AbortSignal is not threaded through paginated gh calls (a cancelled publication can hold the per-PR lock through a scan). Evidence: 408 unit tests (`node --test tests/*.test.mjs`, incl. the fake-gh publish matrix — gate refusals with zero POSTs, one-POST payload shape with pinned `commit_id`, anchor mapping incl. the 50-cap/patchless-file/stale cases, marker forgeries incl. filenames and foreign reviewers, base-advance-since-capture and during-publication fail-closed, malformed-base fail-closed, cancelled-review refusals at entry and pre-POST, per-comment cap, exactly-at-cap pagination, 4xx no-retry vs 5xx reconcile vs uncertain-fail-closed, non-array/oversized fail-closed, renderPublishResult — plus the select/inspect regressions incl. C1) + smoke-i1/i2 green (SDK dispatch, zero inference; i1 asserts `Version: 0.2.4`) + smoke-l1 dry-run green; smoke-i3 (real lanes + adjudication) runs as the loop's assessment gate against this very PR by design. | I6 |
| M1 | ✅ Done (PR #42) | **Generic Copilot plugin marketplace (user-commissioned 2026-09-13, non-plugin increment):** a public, GENERIC marketplace repo `xpepper/copilot-plugins` — designed to host entries for ALL the user's Copilot plugins, z-pr-review first — so installation is two commands: `copilot plugin marketplace add xpepper/copilot-plugins` + `copilot plugin install z-pr-review@copilot-plugins`. Verified mechanics (GitHub reference marketplace `github/copilot-plugins` + local CLI 1.0.48 help): manifest is the only required component at `.github/plugin/marketplace.json` (`.claude-plugin/` alternate accepted); top-level `name`/`metadata{description,version}`/`owner{name,email}`/`plugins[]`; entries carry `name`, `description`, `version`, `author{name,url}`, `homepage`, `keywords[]`, `license`, `repository`, `source`; external-repo sources use the object form `{"source":"github","repo":"xpepper/pr-review-glm","path":"."}` (verify live that a root `path` is accepted — the reference impl only shows subdirectory paths; docs: `docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/plugins-marketplace`). Scope: create the marketplace repo (public; conventional session work, user-commissioned — NOT loop-gated); register z-pr-review pinned to this repo with entry version == `plugin.json`; marketplace repo README; in THIS repo: README/AGENTS install-path docs, and a no-inference marketplace-consistency smoke (entry exists, points at this repo, entry version == `plugin.json` version) wired into the gate smokes with a failure message pointing at the marketplace repo; release discipline documented and gate-enforced: every increment that bumps `plugin.json` also bumps the marketplace entry (the consistency smoke fails the next assessment if skipped). Hazards to document and verify: uninstall any locally installed copy before `--plugin-dir` dev sessions (double registration = the I1 dispatch-ambiguity class); whether installed-from-marketplace plugins still need `--experimental` on Copilot CLI 1.0.83; marketplace-repo commits are direct pushes to a no-gates repo — keep them one-line version/entry changes, disclosed in the increment PR. Evidence: marketplace live at xpepper/copilot-plugins (manifest name `xpepper-copilot-plugins` — the CLI REJECTS a marketplace named `copilot-plugins`, colliding with the built-in default); root `path: "."` ACCEPTED; `source.ref` tag pinning HONORED (`plugin update` tracked v0.2.3→v0.2.4 exactly); marketplace-installed plugins need NO `--experimental` on 1.0.83; end-to-end add→install→registration→status dispatch (`Version: 0.2.4`) verified live then UNINSTALLED locally; in-repo: `tests/smoke-m1.mjs` consistency smoke (entry present, points at this repo at root, version == plugin.json, ref == vX.Y.Z; fetched via the always-fresh contents API — the raw CDN lags ~5m and would flake the gate) wired into gateSmokes + 8 unit tests (`tests/marketplace.test.mjs`); README/AGENTS install-path docs; plugin.json 0.2.5; entry bumped to 0.2.5/v0.2.5 in the same increment. STEP 0 re-landed #38's smoke-retry fix first (#41). Supervised conventional landing (loop environment-blocked: 3/3 shell-less preflight probes, 2026-09-13 22:49). | I7 |
| I8 | ✅ Done (PR #44) | **Hardening (pre-1.0 closer):** large-diff file-backed transport — `extensions/z-pr-review/transport.mjs`: diffs ≥200,000 bytes (spec constant `FILE_BACKED_THRESHOLD_BYTES`) stop riding in every lane prompt; the frozen capture is split into per-file unified-diff sections (0600 files under a 0700 mkdtemp dir), each lane/adjudicator prompt becomes a manifest form (changed-file list + new-side changed line ranges — the SAME anchor contract `parseDiffAnchors` validates with — + each section's transport path) with a REQUIRED-READS rule, and completeness is enforced from tool events at file granularity (the permission request carries a path, not ranges — documented honestly): a lane reaching `session.idle` without reading every required file is a FAILED lane with a disclosed unread-files reason, its findings not claimed; per-attempt read tracking (a fallback attempt re-reads from a fresh set); the transport dir joins the readable roots (repo checkout stays readable); the adjudicator gets the manifest form + readable transport WITHOUT coverage enforcement (merging candidates needs targeted reads; its output is re-validated host-side regardless); manifest/diff consistency is checked fail-closed (repeated paths, anchorless sections, vanished transport files refuse the review closed — never a silent fallback to an embedded multi-hundred-KB prompt). Lane/credit telemetry from runtime events: `model.call_finished` (call count, outcome tallies, summed `dispatchDurationMs`) and `session.usage_checkpoint` (`totalNanoAiu` last−first = the fresh child session's own spend) accumulate per attempt — informational only, never a gate input; only finite non-negative numbers enter results (unknown event shapes skipped, outcome strings classified against the four known values, never copied); rendered per lane in-chat (`[3 model calls, 18.5s model time, 2.50 AIU]`) and as ADDITIVE machine-block keys (`transport{mode,files,diffBytes}`, `lanes[].telemetry{calls,dispatchMs,usageNanoAiu}` — the loop contract ignores unknown keys). Riding work, dispositioned against code first: [I7 P2, valid→fixed] uncertain-write reconciliation is now bounded-retried (3 scans, 2s apart — a lagging review listing can no longer fail-closed over a landed review and bait a duplicating rerun; the reconciling scan number is disclosed in-chat); [I7 P2, valid→fixed] the review's AbortSignal is threaded through every gh call and between pagination pages (a cancelled review stops paging immediately instead of holding the per-PR lock through a scan; checked at the ghJson boundary so every scan funnels through it); [I7 P2, valid→fixed] smoke-i3 target skew — `gateSmokes` threads the worked increment as `SMOKE_INCREMENT` in the smoke child env and smoke-i3 selects via the gates' own `selectIncrementPr` (one selection contract; a stacked/other increment PR can no longer skew the smoke off the assessed PR; manual runs keep the generic default); [loop flag, valid→fixed] probe-transcript fake-stomp — `runPreflightGates` threads `artDir`/`persist` into `gateZcodeHeadless`, and the preflight tests inject a temp dir (root cause found live: the 2026-09-14 14:43 suite run overwrote the real Sep-13 probe evidence with fixture text); [loop flag, fixed] fixer budget 2→4 with a severity ladder (rounds 1–2 carry blocking findings only — gate P0s and review P0/P1; rounds 3–4 widen to all findings; a blocking verdict with no P0/P1 keeps the full list — never an empty fixer dispatch; the V1/M1 pain: two dogfood P1s exhausted a 2-round budget and forced supervisor folds); [loop flag, decided+documented] release tags are ANNOTATED — matching every existing tag v0.2.0–v0.2.5 on origin (the old "lightweight by design" comment described a path that never shipped); `verifyBumpAtMerge` reserves via a locally created annotated tag pushed create-only and returns the tag OBJECT oid, `tagMergedRelease` retargets with `-f -a -m` under `tag.gpgsign=false` (gpg/editor stays unreachable from the headless tail) with the force-with-lease pinned to the reserved tag object, and a refused merge releases BOTH the remote ref and the local tag; [calibration, no data→no change, documented] `PHASE_LIMITS.dogfood` stays 20m with the arithmetic recorded (≈13.5m worst case: 12m batch + 60s adjudication + capture/dispatch overhead) — every run that would have carried observed phaseTimings stopped at the zcode-headless preflight, so the first loop-owned run's timings either confirm or tune it. Deferred docs folded: the I7 fixer-commit and #38/#39 journey entries (below). Manual verification (disclosed): one real light lane over a synthetic 240,503-byte 8-file diff — file-backed transport activated at the default threshold, the lane read ALL 8 required files through the permission handler (completion is impossible otherwise), satisfied the envelope contract, and produced live telemetry (3 calls, 18.0s dispatch; both usage checkpoints carried identical totals so the derived spend degraded honestly to 0 — the checkpoint cadence is coarser than per-call; recorded as a known observation). Supervisor fold 1 (dogfood round 1 on the increment head — verdict approve-with-nits, 6 P2s, ALL validated real; 5 fixed, 1 partial-fix+documented): the fixer ladder's rounds 1–2 keep P0/P1 only (P3/nit from an independent reviewer are not blockers under reviewBlocking's own definition); headerless diff sections (binary "Binary files … differ", mode-only changes) take their path from the `diff --git` boundary line instead of refusing the whole review (a large PR touching any binary file could never be reviewed — anchor-surface consistency relaxed accordingly: headerless sections legitimately have no anchor entry); the telemetry baseline is 0 when the first checkpoint arrives after dispatches (the total already includes the fresh session's own spend — the delta no longer silently drops it); the reconciliation backoff is abort-aware (a cancelled review does not sit out the 2s delay); transport section writes are sequential (no unbounded concurrent fs ops); and the transport directory is removed best-effort when the review settles (publication validates against the live files API, never the transport files; the CAPTURE file stays by I2-era design — inspect names it — documented rather than changed). Supervisor fold 2 (dogfood round 2 on the folded head — approve-with-nits again, 4 P2s, all validated real): the transport-directory cleanup moved into runReview's finally (a throwing or cancelled review leaked it — and a TDZ bug caught live by smoke-i3 made the point: `let transport` mid-try meant the finally's ReferenceError REPLACED a failing review's real outcome; the declaration is hoisted now); a failed transport section-write removes the partial directory before failing closed (injectable writer); the abort signal reaches the gh boundary DURING in-flight calls (post-call check in ghJson + SIGTERM→SIGKILL in defaultRunGh — a cancelled publication no longer waits out a 30s request timeout holding the per-PR lock; a killed POST is an uncertain write the marker reconciliation owns); and the adjudicator path (no coverage enforcement) still verifies every transport file EXISTS before dispatch — host re-validation anchors against the captured diff, which would otherwise still name a vanished section's file. Supervisor fold 3 (dogfood round 3 — approve-with-nits, 5 P2s, all validated real): positional diff headers in BOTH parsers (a "++ text" added line inside a hunk renders as "+++ text" — textually identical to a file header; only lines before a file's first `@@` count now — this fixed a live-since-I5 phantom-path ambiguity in `parseDiffAnchors` too, where hunk content could register phantom files as anchor-touched); leading diff preamble attaches to the first real section instead of forming a pathless section (comment/behavior divergence); the reconciliation scans are deliberately signal-free (cancellation blocks writes and waits, never READS — an uncertain POST the abort itself may have killed still reconciles to the truth now, with the abort-aware backoff stopping further waits); the reported lane telemetry is the SUM over attempts (a failed primary's spend is no longer dropped when the fallback completes); and transport building honors the review's cancellation and total budget between writes (fail-closed + partial-dir cleanup). Supervisor fold 4 (dogfood rounds 4/4b — the FIRST REAL file-backed review: this PR's own diff crossed 200 KB mid-folding, so the dogfood exercised the transport in production — 5 lanes × 27 manifest files, coverage enforced live): heavy lanes starved under inline-tuned budgets (a 26-of-27-files lane died re-reading on the 180s fallback cap); file-backed batches and attempts now carry a bounded proportional read allowance (`transportReadAllowanceMs`: 10s per manifest file, capped at +3m, ZERO without a transport — inline deadline semantics unchanged), and the PHASE_LIMITS.dogfood arithmetic note updated (≈16.5m worst case file-backed, still under the 20m cap). Residual recorded for V2: one lane per run still flaked with empty output under the big read load (the PR #23 class) — model-side stamina under file-backed mode is a ground-testing item, not a code fix. Supervisor fold 5 (dogfood round 5 — approve-with-nits, 5 P2s: 3 fixed, 2 dispositioned design-inherent): the adjudication deadline's total-budget clip widens by the SAME transport read allowance the batch got (a file-backed batch that spent its allowance otherwise left adjudication nothing — a degraded review from budget accounting, not adjudication); a reservation whose tag object fails to resolve after the push is RELEASED (remote + local) instead of stranding the version on a half-taken reservation; transport construction checks cancellation/deadline at ENTRY, before the parsing pass. Design-inherent, documented: shutdown's bounded drain (2.5s) can exit before the best-effort transport-dir removal lands (the rm is best-effort by policy, the dir is 0600 in OS temp with the same exposure class as the deliberately-kept capture file, and unbounded shutdown waits would trade a temp dir for a hung exit); and a cancelled publication holds the per-PR lock through the deliberately signal-free reconciliation scans (round-3's documented trade — reporting an uncertain POST's truth beats releasing the lock sooner; the scans are read-only and bounded by RECONCILE_SCANS × pagination, with the abort-aware backoff cutting the waits). Supervisor fold 6 (dogfood round 6 — the allowance fixed the lanes; the batch completed 5/5 for the first time under file-backed mode, and the ADJUDICATOR then died at 54s of the 60s inline-tuned cap reading the same 27-file manifest): the adjudication window widens by the transport read allowance too (same pattern as the batch — file-backed mode scales its time budgets by the manifest; the total clip still binds). Evidence: 473 unit tests (`node --test tests/*.test.mjs`, incl. transport.test.mjs — section splitting, threshold decision, manifest/ranges vs the anchor contract, prompt shapes, consistency refusals; lane telemetry matrix incl. malformed events; coverage enforcement incl. unrelated-reads and missing-file fail-closed; adjudicator exemption; renderReview transport/telemetry + machine-block keys; publish reconciliation-sequence and cancellation matrices; gates smoke-env threading + preflight no-stomp; loop severity-ladder matrix; annotated reservation/retarget arg shapes) + smoke-i1/i2 (SDK dispatch, zero inference; i1 asserts `Version: 0.2.6`) + smoke-i3 against this very PR + smoke-m1 (marketplace entry 0.2.6/v0.2.6) + smoke-l1 dry-run green. Landed supervised conventional (user's explicit choice at session start, 2026-09-14 — the loop-owned release milestone moves to the 1.0.0 era). | M1 |
| V2 | ✅ Done (PR #47) | **Ground-testing feedback round (0.2.6 → 0.2.7):** four REAL reviews of this repo's merged increments (#43, #41, #37, #44) run through the INSTALLED marketplace copy — a new driver `tests/groundtest-v2.mjs` over a `pluginDir: null` harness option (`startPluginSession` spawns the CLI with no plugin flags, so config discovery loads exactly the marketplace artifact a released-version user runs; never a second registration beside it) — covering both transport paths (`#44`: 216,969-byte diff, 27 manifest files, file-backed transport live with required-read coverage enforced; `#37` 131,705 bytes inline; `#41`/`#43` small inline). **Flake-rate item answered with data: 20/20 balanced lane attempts complete, ZERO flakes** — including the file-backed scenario class where I8 observed ~1 lane/run; the fail-closed partial design stays exactly as-is, no retry/tolerance semantics change warranted on this evidence (recorded for the 1.0.0 decision). **Telemetry: non-zero per-lane spend on every lane of every run** (2.7–58 AIU/lane, 1–7 calls) — the I8 identical-checkpoint observation is the coarse-cadence exception, not the norm. **Budgets: review wall-clocks 9.4s/67.9s/175.9s/215.6s** (file-backed 3.6m vs the ≈16.5m documented worst case and the 20m dogfood cap — the arithmetic holds with wide margin; no tuning warranted). Findings (10 P2s across the four reviews) dispositioned against code first: [valid→fixed] `gateSmokes` now discloses BOTH attempts' tails when a smoke fails twice (the retry's triage value — which attempt produced which diagnostic — was lost; the #41 review's finding on #41's own subject); [valid→fixed] refused-merge tag cleanup is now OWNERSHIP-VERIFIED (`releaseTagReservation` in version.mjs, wired at BOTH release sites): the remote delete is leased to the reserved tag OBJECT oid (verified live: git honors `--force-with-lease` on `--delete` pushes; the expect value for an annotated tag ref is the tag object oid — a commit oid is silently wrong), the local tag deletes only on a matching object, and the resolve-failure path verifies by peeling the remote object to the reserved commit before deleting — a tag another actor moved is left in place and disclosed, never deleted (the #44 review's P2); [valid→fixed] `runLaneBatch` accepts `reviewDeadlineAt` and min()s its batch/total windows against it (extension.mjs computes it once from the REVIEW's start + totalMs + transport allowance; the adjudication clip reuses the same value — file-backed transport-build time can no longer restart the batch's total clock past the window the review's own accounting uses); [design-inherent, documented, no change] shutdown's bounded drain vs best-effort transport-dir removal (I8 fold 5's recorded trade — the #44 review restated it); [fail-closed by design, documented] unbounded manifest file-counts vs the read allowance (lanes that cannot complete required reads fail → partial → blocked; the ≥200 KB threshold itself is settled); [settled design, documented] the idempotency marker keys on the capture binding (I7 fold 3 — one COMMENT per capture binding IS the contract; re-selection re-publication at an unchanged head is post-v1 UX); [already fixed on main, 2 findings] the #37 review described pre-I8 publish.mjs (signal-aware pagination and bounded reconciliation landed as I8's riding P2s — reviewing a MERGED PR reports its tree as it was then; an expected, now-observed property of ground-testing old diffs). **Coverage approval≈read candidate: verified FEASIBLE on SDK 1.0.83 but NOT landed** — `tool.execution_start.data` carries `toolName`+`arguments`+`toolCallId` and `tool.execution_complete.data` carries `success`+`toolCallId` (joinable for successful reads); correlating SUCCESSFUL reads makes the completeness gate stricter, which interacts with the flake-sensitivity design surface — deferred to the user's call (HANDOFF carries it). Supervisor fold 1 (dogfood round 1 on the increment head — 5/5 lanes complete, 3 findings: 1 P1 + 2 P2s, ALL validated real and fixed): the ground-test driver derived its capture-file cleanup path from the model-influenced FINDINGS report (`report.match(/Capture file: …/)`) — a crafted finding detail could steer `rmSync` at any writable path (P1); the path is now trusted ONLY from the code-rendered capture summary and only inside an `z-pr-review-`-prefixed mkdtemp dir, and the same shape in `smoke-i3`'s fallback (pre-existing, disclosed riding fix) got the identical treatment — smoke-i2 already isolated the capture message and needed nothing; a FAILED `git ls-remote` in the resolve-failure release read as an absent tag, reporting success while the remote reservation stranded (P2) — a failed lookup is now a failed release carrying the error; and the peel-match ownership check accepted any remote tag peeling to the reserved commit — a replacement tag from another actor pointing at the same commit would be deleted (P2) — the object must now ALSO exist in the local object store (this run created it; a remote-only replacement did not), with the shared-checkout concurrency residual documented (verifyBumpAtMerge's local-tag pre-check serializes concurrent runs). Supervisor fold 2 (dogfood round 2 on the folded head — 5/5 lanes complete, 1 P2, validated real): the resolve-failure path's LOCAL `git tag -d` ran unconditionally — the local twin of fold 1's remote finding (another actor may have created/moved the shared checkout's tag while this run's ref was unresolvable); a RESOLVABLE local tag is now deleted only when it matches the peel-verified object, otherwise left in place and disclosed (unresolvable tags still delete as a no-op). Supervisor fold 3 (dogfood round 3 on the fold-2 head — 5/5 lanes complete, 3 P2s, ALL ONE FAMILY: TOCTOU windows on LOCAL tag refs — check-to-delete races git cannot lease): the family had now repeated three rounds running (round 1 remote-side, rounds 2–3 local-side), the escalation trigger under the fold-and-converge policy; disposition = DOCUMENTED DESIGN RESIDUAL, no fourth fold — the remote (the artifact releases depend on) is protected ATOMICALLY by the lease + create-only reservation push, while local refs have no compare-and-swap and the windows are sub-second inside an already-rare path; the boundary is recorded in releaseTagReservation's doc comment as the family's standing disposition. Verdict of record: P2-only, meets the merge policy. Evidence: 485 unit tests (+12: the two-attempts smoke disclosure incl. a differential transient-vs-real case, the `releaseTagReservation` matrix — leased arg shapes, rejected lease, moved local/remote tags left in place, peel-verified delete, absent-remote no-op, failed-lookup fail-closed, locally-unknown replacement left alone, unprovable-local left alone (both sub-cases) — and the review-level batch cap) + smoke-i1/i2 (asserting `Version: 0.2.7`) + smoke-i3 against this very PR + smoke-m1 (entry 0.2.7/v0.2.7, e5807e5 on xpepper/copilot-plugins, rebased over the sibling's d0a471e — coordinated, not stomped) + smoke-l1 dry-run green. Landed supervised conventional (user unavailable at session start; the proven I7/M1/I8 path — the FOURTH supervised landing; the first loop-owned release milestone stays parked for the 1.0.0 era). | I8 |
| R45 | ✅ Done (PR #49) | **Marketplace release-process fix (issue #45 — closes the missing-tag window; process/tooling only, NO plugin.json bump):** the M1 discipline required the public marketplace entry (xpepper/copilot-plugins) to equal `plugin.json` BEFORE an increment could merge, forcing the entry's `source.ref` to name a release tag that is only pushed at the merge — a window (as long as the increment takes to merge) where `copilot plugin install/update` points at a missing ref (observed live 2026-09-15 with the entry at 0.2.7 before tag v0.2.7 existed; the acute case was resolved same day by pushing the tag — R45 is the structural fix). Changes: (1) pre-merge gate `tests/smoke-m1.mjs` (inside `gateSmokes`) now accepts the live entry at EITHER `plugin.json`'s version (an operator may bump early) OR the last released tag (highest existing `vX.Y.Z`, numeric max via `compareVersions`/`isStrictVersion` from `scripts/dev-loop/version.mjs` — reused, not forked; entry `version` and `source.ref` must agree with each other); the pure rules moved to the new `scripts/dev-loop/marketplace.mjs` and are re-exported by the smoke (one import surface for tests); the manifest is still read via the GitHub contents API (raw CDN lags ~5m). (2) The entry bump moved to a POST-MERGE step of the loop's merge tail: `bumpMarketplaceEntry` (same module), wired in `scripts/dev-loop.mjs` after `tagMergedRelease` pushes the tag — bumps `version`+`source.ref` together via a sha-pinned contents-API PUT (one fresh-refetch retry on rejection — the pull --rebase equivalent; the bump re-applies onto the FRESH manifest so sibling entries are never stomped; never force), then verifies the live entry and fails loudly with the tag name if the pinned ref is missing on origin; idempotent on re-run (an entry already at the released version commits nothing); a failed bump FAILS the merge path (the release is unpublished, never silently skipped — the next gate would accept the stale entry as the last-release arm and the skipped bump would ship invisibly). V2's `releaseTagReservation` lease semantics untouched (dispositioned residual family). Evidence: unit suite green with 60 new/reworked tests (either-or rule matrix + tag-fetch fail-closed in `tests/marketplace.test.mjs`; `bumpMarketplaceEntry` happy/idempotent/tag-missing/retry/verify matrix + `applyEntryBump` surgical-diff + `highestReleaseVersion` in the new `tests/dev-loop-marketplace.test.mjs`; `isStrictVersion` pin in `tests/dev-loop-version.test.mjs`); `node tests/smoke-m1.mjs` green live (entry 0.2.7/v0.2.7 accepted as the last-release arm while this branch carries no version bump); `node tests/smoke-l1.mjs` NOT run — it transitively executes the SDK smokes (unsafe beside the parallel #46 session) and was deferred to the supervisor's serialized integration pass; AGENTS.md release-discipline passage updated to the post-merge flow; the marketplace README's stale z-pr-review row (0.2.5) fixed to 0.2.7/v0.2.7 by disclosed direct push. | V2 |
| S46 | ✅ Done (PR #48, merged 2026-09-15; released as 0.2.8 under the R45 post-merge flow) | **Self-review publication opt-in (GitHub issue #46; 0.2.7 → 0.2.8):** `--comment --self-review` — the explicit, deliberate pairing that lets the PR's own author publish the tool's single COMMENT review to their PR. The self-author gate stays fail-closed for everything else, and its refusal reason now names the opt-in. The parser accepts `--self-review` ONLY with `--comment` (alone, with `--no-comment`, with `--capture-only`, or repeated are usage errors), so no ordinary-flag combination can smuggle the bypass and config `autoPostReviews` — which grants publication exactly when NO comment flag was passed — structurally cannot imply it (config-driven publication still hits the self-author refusal, per the brief's settled config rule). extension.mjs threads the flag to `publishReview` as code-owned `allowSelfReview` (default false); the gate opens only when the viewer authored the PR AND the authorization is present, and the posted COMMENT body then carries a static code-owned disclosure (`SELF_REVIEW_DISCLOSURE`: published by the PR's own author, explicitly authorized at invocation time by the --self-review flag) with the chat render disclosing too — the disclosure appears only on a factual self-review, never on someone else's PR. Every other gate unchanged (lifecycle/draft, stale head/base degradation, anchor re-validation, cancellation, idempotency/duplicate, pre-write re-checks, per-target write serialization — pinned by tests that the opt-in does not bypass the draft gate or the missing-author fail-closed). Release sequencing deliberately split from the then-current M1 same-increment rule: plugin.json bumped to 0.2.8 in-PR while the marketplace entry bump + annotated `v0.2.8` tag stayed with the supervisor at merge time (the R45 release-machinery increment ran in a parallel worktree and landed FIRST, so the S46 branch rebased onto the new either-or gate — smoke-m1 green via the last-release arm with 0.2.8 in flight, then 528/528 after rebase). Landed via the supervised parallel-subagent session (worktree-per-issue, fresh task-reviewer per branch, dogfood through the installed copy — both PRs' folds recorded in their conversations); release completed at merge: annotated tag `v0.2.8` peeling to the merge commit, entry bumped to 0.2.8/v0.2.8 by the FIRST LIVE execution of R45's `publishTaggedVersion` (clean, verified by re-read). Evidence: 494 unit tests on the branch (+9 over the 485 baseline: parse acceptance/rejection matrix incl. the structural no-`--comment` guarantee, opt-in publish with the body disclosure + unchanged idempotency marker, opt-in no-op when the viewer did not author the PR, draft/missing-author still refused under the opt-in, stale+disclosure composition, chat-render disclosure, help/status surface) + smoke-i1/i2/i3 (i3: real 5-lane review of PR #48 through the branch copy, parent inference-free) + smoke-m1 (last-release arm) — all green. | V2 |
| V3 | ⬜ Pending | **The 1.0.0 release decision round — the USER's call, never automatic** (1.0.0 has been deferred since 2026-09-14 pending satisfying ground testing): the session lands the decision conversation on the V2 data (0/20 lane flakes, non-zero telemetry, transport live and well inside budgets), and either bumps 0.2.7 → 1.0.0 (marketplace entry in-lockstep per the M1 discipline) or iterates (V4+). Carried candidates for the user to disposition: coverage successful-read correlation (feasibility verified in V2; strictly tightens the completeness gate — interacts with flake sensitivity), re-selection re-publication at an unchanged capture binding (post-v1 UX today; changing it reopens the I7 fold-3 marker design), and the first loop-owned release milestone (parked since I7 — needs a healthy zcode-headless environment and the user's terminal launch). | V2 |
...
- **2026-09-15 (S46 — self-review publication opt-in; implementer session, PR
  #48)** — the solo-maintainer gap from GitHub issue #46: `--comment` refused
  publication on the user's own PRs, leaving self-reviews chat-only. Landed the
  `--comment --self-review` pairing as the explicit authorization. The load-
  bearing decision is WHERE the authorization lives: not in publish.mjs logic
  alone but in the parser — `--self-review` without `--comment` is a usage
  error, so config `autoPostReviews` (which grants publication exactly when no
  comment flag was passed) structurally cannot carry the bypass, satisfying the
  issue's "explicit and distinguishable" requirement by construction rather
  than by convention. The authorized publication discloses itself: a static,
  code-owned `SELF_REVIEW_DISCLOSURE` line in the posted COMMENT body (same
  authority class as the idempotency marker) plus a chat-render line; the
  disclosure fires only on a factual self-review (viewer === author), never on
  someone else's PR. All other gates pinned unchanged by new tests (draft and
  missing-author still refuse under the opt-in; stale degradation composes with
  the disclosure). Version 0.2.8 on the branch; the marketplace entry + tag are
  deliberately NOT in this PR — the supervisor sequences them at merge (the #45
  release-machinery increment, parallel worktree), smoke-m1 was deferred to the supervisor's serialized pass and is green after the R45 rebase (last-release arm: entry 0.2.7/v0.2.7 accepted while 0.2.8 is in flight). HANDOFF untouched for
  the same reason (supervisor rewrites it at final landing).

- **2026-09-15 (R45 — marketplace missing-tag window; implementer subagent in a
  parallel-worktree session, #46 feature work disjoint by ownership)** — the M1-era
  discipline made the public marketplace entry equal `plugin.json` BEFORE merge,
  so the entry's pinned `source.ref` named a tag that did not exist until the
  merge pushed it: an install/update window pointing at a missing ref (observed
  2026-09-15 at entry 0.2.7/tag absent; the tag was pushed the same day — the
  structural fix is R45). The entry bump moved to a POST-MERGE step of the merge
  tail (`bumpMarketplaceEntry` in the new `scripts/dev-loop/marketplace.mjs`:
  contents-API read → surgical two-line entry bump (version+ref together,
  canonical-shape round-trip guard, siblings untouched) → sha-pinned PUT with one
  fresh-refetch retry (the API's pull --rebase; never force) → verify, failing
  loudly with the tag name if the pinned ref is absent on origin), and the
  pre-merge smoke now accepts EITHER `plugin.json` OR the last released tag
  (highest existing vX.Y.Z, numeric — reusing version.mjs's comparison, not
  forking it). A failed bump fails the merge path rather than warning: the next
  gate would accept the stale entry (arm 2), so a silent skip would strand
  releases unpublished. The marketplace repo's README z-pr-review row was also
  fixed to 0.2.7/v0.2.7 (one-line disclosed direct push). No plugin.json bump
  (process change only); #46's 0.2.8 lands separately.
- **2026-09-14 (V2 — ground-testing feedback round; supervised conventional, user
  unavailable at session start)** — four real reviews through the INSTALLED
  marketplace copy 0.2.6 (new `tests/groundtest-v2.mjs` driver + a `pluginDir: null`
  harness option: config discovery loads the marketplace artifact — the real-user
  loading path, no second registration, no uninstall dance). The round's headline
  answers: **0/20 lane flakes** (the I8 ~1-lane/run observation did not reproduce,
  including the file-backed 27-file case — fail-closed partial semantics stay as
  designed), **non-zero per-lane telemetry everywhere** (the 0-delta reading is the
  coarse-cadence exception), **file-backed wall-clock 3.6m vs the 20m cap** (budget
  arithmetic holds with margin; no tuning). Ten P2 findings across the four reviews,
  all validated against code first: three fixed (smoke double-failure disclosure —
  fittingly found BY the tool reviewing #41, ABOUT #41's own retry feature;
  ownership-verified tag-reservation release at both sites, with the leased-delete
  git semantics verified live in a scratch repo first; the review-level batch
  deadline that file-backed transport-build time used to escape), two already fixed
  on main (the #37 review describing pre-I8 publish.mjs — merged-PR reviews report
  the then-tree, now an observed ground-testing property), and five
  design-inherent/settled (documented in the row). The coverage approval≈read
  candidate was verified feasible against the bundled SDK's event types
  (`execution_start` arguments + `execution_complete` success, joined by
  `toolCallId`) and deliberately NOT landed: it strictly tightens the completeness
  gate while the flake question it interacts with had just been answered with
  zero-flake data — the user dispositions it at the 1.0.0 decision. Version: landed
  as **0.2.7** (marketplace entry 0.2.7/v0.2.7 at e5807e5, rebased over the
  sibling's same-day d0a471e — the push was rejected first and pulled-with-rebase,
  the never-stomp discipline holding under live contention). HANDOFF moves to
  `next=V3` (the 1.0.0 decision round).

- **2026-09-14 (I8 — hardening; supervised conventional, user's explicit path choice)** —
  file-backed transport + telemetry + the riding P2/flag fixes, all dispositioned against
  code first (row above). Session-start decisions recorded: I8 lands as **0.2.6** (marketplace
  entry bumped in-lockstep, 4ec4448 on xpepper/copilot-plugins) with **1.0.0 deferred** until
  the user is satisfied from ground testing — hence the new V2 row and `HANDOFF next=V2`; and
  the **third supervised landing** (I7, M1, I8) moves the first-loop-owned-release milestone
  to the 1.0.0 era. Scope notes: the module cycle transport↔adjudicate↔lane is deliberate
  (all cross-imports are call-time — safe under ESM live bindings in any load order); the
  coverage check is file-granular because permission events carry paths, not line ranges —
  the manifest's ranges remain the lane's guidance, and that boundary is documented rather
  than papered over. The known observation from the manual run: usage checkpoints can repeat
  an identical total across a lane's calls (coarser debit cadence), so the derived per-lane
  spend reads 0 — telemetry never invents a delta.

- **2026-09-13 (deferred record: the I7 fixer commits and the #38/#39 base incident —
  promised then, folded here per the I8 docs pass)** — two things the journey log owed. (1)
  **The I7 fixer commits were loop-owned design work**: round 1 (b029c48) implemented
  SELECTION-CARRY (an outgoing select-settled selection for the same PR becomes the
  publication queue under ambient autoPostReviews authority; `--all` overrides; full chat
  disclosure) plus the round-4 P2s; round 2 (dd8de51) fixed the LOOP ITSELF —
  `selectIncrementPr` prefix-scoped PR selection wired into gateIncrementPr, the assessment
  inline gate, findResumablePr, and smoke-i3 — after the spec's "exactly one open PR FOR THE
  INCREMENT BRANCH" had been over-tightened to repo-wide. Systemic lessons that now bind:
  a fixer can repair loop code, but the RUNNING process can never load it (mid-run loop-side
  fixes are inert until a process launched from a tree containing them runs), and loop-side
  PRs must land BETWEEN runs, never while one is live (they trip the increment gates
  mid-assessment and burn fixer budget on loop code). (2)
  **The #38/#39 base incident**: #38 (the smoke-retry+stderr fix, branch cut from the I7
  branch) was supervisor-merged on precedent WITHOUT checking the PR's base — the squash
  carried ALL of I7 into main under the smokes-fix message (+1430 lines, plugin 0.2.4,
  ROADMAP I7 ✅, HANDOFF next=I8) while #37 was still open. Contained by revert PR #39
  (tree verified byte-identical to the pre-merge head); the smoke fix was re-landed fresh
  from main as #41. Standing policy since (the #34 discipline, skipped once at real cost):
  before ANY supervisor merge, verify the PR branch's BASE (base.sha == current main head)
  and that the diff vs main is surgical and matches the PR body — "verified/tests green"
  never covers base sanity. #38's real content owed a fresh main-based PR between runs;
  that debt was paid at M1 (STEP 0, #41).
- **2026-09-13 (M1 — supervised conventional landing; loop environment-blocked)** —
  the user's 22:49 `--merge auto --dogfood on` launch stopped at the `zcode-headless`
  preflight: 3/3 probes shell-less (sessions AND subagents — the degraded-evening
  signature; transcripts under `.dev-loop/`), so the proven I7 fallback applied and
  the supervisor session delivered M1 conventionally. STEP 0 first: PR #38's
  smoke-retry fix (lost in the #39 revert) re-applied fresh onto main with the delta
  verified content-identical (#41, 411/411). The marketplace was then built LIVE
  against Copilot CLI 1.0.83: a manifest named `copilot-plugins` is REJECTED
  (built-in default collision) → `xpepper-copilot-plugins`; root `path: "."` installs
  fine; `source.ref` pinning is honored (probe: entry pinned v0.2.3 downgraded the
  installed copy 0.2.4→0.2.3, restore to v0.2.4 upgraded it back); installed plugins
  register and dispatch WITHOUT `--experimental`; the installed copy answered
  `Version: 0.2.4` by direct RPC, then was uninstalled before any `--plugin-dir`
  work (the I1 double-registration class). In-repo: `tests/smoke-m1.mjs` gates the
  release discipline (entry version + ref tag == plugin.json; contents API, not the
  raw CDN — a ~5-minute raw-cache lag failed the smoke right after the entry bump,
  which for a discipline gate is a spurious failure), 8 unit tests, README/AGENTS
  install-path docs, plugin.json 0.2.5 with the marketplace entry bumped in lockstep.
...
- **2026-09-13 (I7 supervisor fold 2 — dogfood round 3, launched from a
  ...
  carrying selections across reviews is post-v1). 405/405 green after the
...
- **2026-09-13 (I6 — selection and retention)** — implemented as designed in
  ...
  `parseSelectionSpec` (total grammar: `all` · `none` · comma lists of numbers
  and ascending ranges; out-of-range/descending/duplicate specs refused with
  precise reasons, never truncated), selection constructors (`default` keeps
  every validated finding unsettled; `--all` settles the same at review time;
  `select` settles any spec), `renderSelectResult`, and `renderInspect`. The
  report numbers findings (`1. [P1] title [lane] — file:line`) — the numbers
  ARE the selection surface, so a selection can only name findings that
  survived host validation, adjudication, and the mode policy — and footers
  the selection instructions; the machine-summary block is byte-identical in
  shape (protocol surface). `extension.mjs` retains the assembled review
  (even partial/degraded — its status travels with it; its findings were
  still host-validated) plus the selection in session state and answers
  `inspect`/`select` with no inference and no GitHub access. Design decisions
  flagged in the PR rather than taken silently: (1) elicitation is the chat
  follow-up turn — the Copilot SDK command-handler surface exposes no
  verified interactive-prompt API, so `/z-pr-review select …` IS the native
  surface available (the spec's "native elicitation" wording names pi's
  mechanism; flagged, not rewritten); (2) `--all` is an explicit settle of
  the default (all findings) — recorded as `via: "--all"` so the retained
  state distinguishes settled-at-review-time from awaiting-selection; (3)
  `select none` keeps the findings visible in `inspect` marked
  not-selected — deselection is a publication posture (I7), not a deletion;
  (4) cross-session persistence of the retained result stays out of scope
  (spec's out-of-scope list). Protocol surfaces untouched (command
  descriptions, machine-summary field shape, STATUS grammar exactly as
  main).
...
- **2026-09-12 (I5 implemented — NOT landed; worker session had no shell access)**
  ...
  PR. The full I5 change set was written to the working tree on `main`
  (uncommitted): new `extensions/z-pr-review/adjudicate.mjs` (diff-anchor
  parsing, host candidate validation, dedup, per-mode policy, adjudicator
  prompt + isolated call, `assembleReview` with degraded paths), a `prompt`
  override on `runLane` (adjudicator reuses the lane machinery), `renderReview`
  reworked for the assembled review (machine-summary field shape unchanged —
  protocol surface), `renderStatus`/`renderHelp` capability text, extension
  wiring (adjudication deadline clipped to the remaining `totalMs`), new
  `tests/adjudicate.test.mjs`, updated `renderReview` tests in
  `tests/lane.test.mjs`, `plugin.json` 0.2.1 → 0.2.2, this ROADMAP row, and a
  HANDOFF rewrite (`STATUS: blocked`). NOTHING IS VERIFIED: no test, no smoke,
  no lint pass ran. Design decisions taken (to be flagged in the eventual PR):
  evidence (non-empty detail) is required for P0/P1 only; file-less findings
  are kept as whole-PR observations (unanchorable inline, I7's problem);
  balanced-mode P3/nit allowance = first 3 diff-anchored (file+line) in order;
  custom (C1) modes get no policy of their own — everything kept, like full;
  adjudicator `sources` naming unknown lanes are stripped (attribution only);
  a partial batch still synthesizes completed lanes' findings (machine
  `status` stays `partial`, so the dogfood verdict still blocks); adjudication
  failure/malformation/budget-expiry degrades to validated candidates with
  review status `degraded` (never reads clean). Next session with shell:
  verify (`node --test tests/*.test.mjs`, smokes i1/i2/l1; smoke-i3 now also
  exercises real adjudication by design), fix what the suites catch, branch
  `i5-validation-adjudication`, commit, push, open the PR, flip HANDOFF to
  `STATUS: next=I6`, and let the loop merge.

- **2026-09-12 (I5 verified and landed by the supervisor after the shell-less
  worker session)** — the supervisor session executed the worker's own handoff
  checklist. Verification: the first full `node --test tests/*.test.mjs` run
  caught ONE real defect the worker's own new tests exposed — an adjudicator
  output entry dropped by lane SHAPING (e.g. an off-ladder severity inside the
  adjudicator's envelope) was counted in NO drop bucket, vanishing from the
  chat report and the machine summary's `dropped`; fixed by folding the
  adjudicator lane's shaping drops into `drops.adjudication` (the test's
  expectation was right, the code under-disclosed). Also removed an unused
  `isReviewMode` import. After the fixes: 330/330 unit tests green (310 before
  + 20 new), smoke-i1 green (status `Version:` line vs plugin.json 0.2.2), and
  smoke-i2 green (SDK dispatch, zero inference). Landing followed the worker's
  instruction exactly: branch `i5-validation-adjudication`, one commit
  carrying the worker's change set + the supervisor fixes, PR opened with the
  incident disclosed; the loop (`--merge auto --dogfood on`) owns the merge
  and the v0.2.2 tag — no agent merges, no agent tags. Open questions flagged
  for the loop's owner rather than decided silently: (1) WHY the worker phase
  session had no shell tool is undiagnosed — same binary (zcode 0.16.5, mtime
  Sep 4), same arg set, and same env code as C1's fully-working run 80 minutes
  earlier; the phase transcript died with the cleaned-up isolated HOME, and
  phase stdout is discarded on exit-code-0; (2) the `zcode-headless` preflight
  probe is a text-only turn, so it cannot catch a degraded phase TOOLSET — a
  tool-exercising probe (and persisting phase stdout) is candidate loop
  hardening; (3) smoke-i3's real adjudication runs first at the loop's
  assessment of this PR — a flaky adjudicator there will surface as a blocking
  dogfood verdict by design (fail-closed), and the fixer owns the fallout.


- **2026-09-12 (C1 landed)** — custom review roles implemented as designed in
  the 2026-09-10 conversation: `roles.mjs` resolves a mode through config —
  `config.modes[mode]` when present (a standard name there overrides the
  built-in topology), otherwise the built-in topology — with each id mapping
  to a custom role (prompt becomes the lane objective; model/effort
  overrides ride on the lane descriptor) or to the built-in lane (first
  definition in quick→balanced→full→deep order, documented as the baseline
  variant so id resolution stays unambiguous). `config.mjs` moved to
  schemaVersion 2 with whole-object validation of `roles` (prompt + tier
  required; model null-or-string; effort enum; role ids must not collide
  with built-in lane ids) and `modes` (non-empty ordered id arrays
  cross-checked against built-in lane ids and defined roles);
  `defaultMode` now accepts custom mode names. `lane.mjs`/`batch.mjs` apply
  lane-level model/effort over the tier's values (the role model is the
  primary and same-model retry; a configured tier fallback still wins as the
  escape hatch). `extension.mjs` resolves the mode through config before
  dispatching the batch; `commands.mjs` surfaces roles/modes in
  `/z-pr-review-config show` (one line per role with a flattened one-line
  prompt excerpt so a prompt can never forge config lines) and documents the
  file-editing workflow in help. Bumped 0.2.1 (additive). Design decisions
  flagged in the PR rather than taken silently: v1 config files are rejected
  by the schemaVersion bump (defaults activate; the file is left untouched)
  — consistent with the R1 starts-fresh posture; built-in ids shared between
  topologies resolve to their first definition; custom modes are selectable
  only via `defaultMode` (mode flags stay standard — a `--<custom-mode>`
  flag grammar would be a new protocol-adjacent surface and is not needed
  for v1).
...
- **2026-09-11 (V1 landed)** — release versioning implemented as designed (no
  ...
  — `gateVersionBump` (wired into the worker gate batch after gateTests and
  BEFORE the inference-consuming smokes, so a bump-less PR fails in seconds;
  ...
  Review-fix pass (same PR, findings on #23): `mergeTail` now polls GitHub for
  the MERGED confirmation (30 × 10s, covering merge-queue/briefly-delayed
  merges) and pins the PR's `mergeCommit.oid` — main's HEAD must be exactly
  that commit before tagging, so a later interleaved merge fails closed instead
  of being tagged as this PR's release; `compareVersions` orders digit strings
  (length-then-lexical) so identifiers beyond 2^53 don't collapse as floats;
  manifest reads in the gate/tail are fail-closed details (never crashes); a
  failed local-tag delete after a failed push is disclosed; `readPluginVersion`
  extracted to `extensions/z-pr-review/version.mjs` and unit-covered;
  smoke-i1 now asserts the status `Version:` line; manifest-backed suites
