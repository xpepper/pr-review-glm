...
| L2 | ⬜ Pending | **autopilot merge mode** (non-plugin increment): `--merge human\|auto` on the dev-loop — the loop itself squash-merges the increment PR when gates are green and all active reviews are clean; merging stays loop-owned and code-governed, never agent-discretion. Default stays human (auto is always an explicit flag); pre-I3 auto = explicit opt-in on the independent review alone; from I3, auto additionally requires the dogfood review (enforced by I3's wiring). Spec: dev-loop design, amended 2026-09-10 (see Amendments). | L1 |
